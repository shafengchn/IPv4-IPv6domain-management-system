...
/**安全退出**/
function IndexOut() {
    top.showConfirmMsg('亲：您是否确认退出系统？', function (r) {
        if (r) {
            window.location.href = 'UserExit.do';
        }
    });
}
//当前日期
...
function Maximize() {
    $("#Header").hide();
    $("#full_screen").attr('src', '../skins/Default/images/16/arrow_inout.gif').attr('title', '还原').attr('onclick', 'Fullrestore()');
...
function Fullrestore() {
    $("#Header").show();
    $("#full_screen").attr('src', '../skins/Default/images/16/arrow_out.gif').attr('title', '最大化').attr('onclick', 'Maximize()');
    MainContent_subtract = MainContent_subtract + 70;
    Sidebar_subtract = Sidebar_subtract + 70;
    iframeresize();
    $("#side_switch").css("top", $('.leftselected').offset().top);
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace) {
   // SetSystemId(tabid);
    if (url == "" || url == "#") {
        ...
        if(index1>0){
            url=url+"&IframeId=tabs_iframe_"+tabid;
        }else{
            url=url+"?IframeId=tabs_iframe_"+tabid;
        }
    }
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','true')\"><a href=\"javascript:;\"><img src=\"/skins/Default/images/32/" + img + "\" width=\"20\" height=\"20\">" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"javascript:AddTabMenu('" + tabid + "','" + url + "','" + name + "','" + img + "','false')\"><a><img src=\"/skins/Default/images/32/" + img + "\" width=\"20\" height=\"20\">" + name + "</a></li>");
        }
...
function AddTabMenu_NoSelect(tabid, url, name, img, Isclose, IsReplace) {
   // SetSystemId(tabid);
    if (url == "" || url == "#") {
        url = "/ErrorPage/404.html";
    }else{
...
//=================动态菜单tab标签========================
function AddTabMenuEx(tabid, url,code, name, img, Isclose, IsReplace) {
    // SetSystemId(tabid);
    if (url == "" || url == "#") {
        url = "/ErrorPage/404.html";
    }else{
        var index1=url.indexOf('?');
        if(index1>0){
            url=url+"&IframeId=tabs_iframe_"+tabid+"&Code="+code;
        }else{
            url=url+"?IframeId=tabs_iframe_"+tabid+"&Code="+code;
        }
    }
