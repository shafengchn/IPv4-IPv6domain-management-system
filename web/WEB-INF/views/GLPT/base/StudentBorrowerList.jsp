...
<div id="tb" style="height: 95px;">
    <div class="tools_bar">
        ...
        <a title="" onclick="fun_Import();" class="tools_btn">
            <span><b style="background: url('/skins/Default/images/16/application_edit.png') 50% 4px no-repeat;">导入</b></span>
        </a>
        <div class="tools_separator"></div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input id="btnSearch1" type="button" class="btnSearch" value="搜 索"
                                           onclick="fun_MFFindEx2('tt1','学生借用者列表'
                                           ,$('#txt_MFFindValue').val()
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="query" id="query" class="select" style="width: 70px">
                                        <option value="Code">学生学号</option>
                                        <option value="FullName">学生姓名</option>
                                ...
                                <td>
                                    <input id="btnSearch" type="button" class="btnSearch" value="搜 索"
                                           onclick="fun_JQFindEx2('tt1','学生借用者列表'
                                           ,$('#query').val()
                                           ,$('#txt_JQFindValue').val()
                                           ,$('#Enabled').attr('checked'))">
                                </td>
...
<script type="text/javascript">
    $(function(){
        $('#tt1').datagrid({
            title: '学生借用者列表',
            fit: true,
            url: '/GLPT/BASE/StudentBorrowerFindPage.do',
            remoteSort: false,
            rownumbers: true,
            idField: 'StudentBorrowerId',
            frozenColumns:[[
                {field:'ck',checkbox:true}
            ]],
            columns: [
                [
                    {field: 'Code', title: '学生学号', width: 80, align: 'center'},
                    {field: 'FullName', title: '学生姓名', width: 100, align: 'center'},
    ...
    var fun_add1=function(){
         var url="/GLPT/BASE/StudentBorrowerAdd.do?IframeId=${IframeId}";
         top.openDialog(url, 'StudentBorrowerAdd', '学生借用者管理 - 添加', 750, 480, 50, 50);
    }
    ...
    var fun_del1=function(){
        ...
        if(row.length>1){
            for(var i=0;i<row.length;i++){
                if(i==0){
                    row1=row[i].StudentBorrowerId;
                }else{
                    row1=row1+"|"+row[i].StudentBorrowerId;
                }
            }
            top.showConfirmMsg('真的要删除选中的['+row.length+']条记录吗？', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/GLPT/BASE/StudentBorrowerBatchThoroughDelOK.do",
                        processData: true,
                        dataType: "json",
                        data: {BatchID:row1},
            ...
            delConfig("/GLPT/BASE/StudentBorrowerThoroughDelOK.do","StudentBorrowerId="+row[0].StudentBorrowerId);
        }else{
    ...
    var fun_edit1=function(){
        ...
        if(row){
            var url="/GLPT/BASE/StudentBorrowerEdit.do?StudentBorrowerId="+row.StudentBorrowerId+"&IframeId=${IframeId}";
            top.openDialog(url, 'EmployeeEdit', '学生借用者管理 - 修改', 750, 480, 50, 50);
        }else{
            showFaceMsg("请选择要操作的记录");
        }
    ...
    var fun_show = function (gridID) {
        ...
        if (row) {
            var url = "/GLPT/BASE/StudentBorrowerShow.do?StudentBorrowerId=" + row.StudentBorrowerId + "&IframeId=${IframeId}";
            top.openDialog(url, 'StudentBorrowerShow', '借用者信息 - 查看详细信息', 500, 430, 50, 50);
        } else {
            showFaceMsg("请选择要操作的记录");
        }
    }
    ...
    var fun_QRCode = function (gridID) {
        var row = $('#' + gridID).datagrid('getSelected');
        if (row) {
            var url = "/GLPT/BASE/StudentBorrowerQRCode.do?EnCode=" + row.Code + "&ImgWidth=200&ImgHeight=200&IframeId=${IframeId}";
            top.openDialog(url, 'StudentBorrowerQRCode', '学生借用者管理 - 生成二维码', 300, 250, 50, 50);
        } else {
            showFaceMsg("请选择要操作的记录");
        }
    }
    ...
    var fun_Import = function () {
        var url = "/GLPT/BASE/StudentBorrowerImport.do?IframeId=${IframeId}";
        top.openDialog(url, 'StudentBorrowerImport', '学生借用者管理 - 导入', 450, 280, 50, 50);
    }

    var fun_ResetPassword = function () {
        var row = $('#tt1').datagrid('getSelected');
        if (row) {
            var url = "/GLPT/BASE/StudentBorrowerResetPassword.do?StudentBorrowerId=" + row.StudentBorrowerId + "&IframeId=${IframeId}";
            top.openDialog(url, 'StudentBorrowerResetPassword', '学生借用者管理 - 重置查询密码', 320, 110, 50, 50);
        } else {
            showFaceMsg("请选择要操作的记录");
        }
    }
