...
<div id="tb" style="height: 96px;">
    <div class="tools_bar">
        ...
        <a title="" onclick="fun_show();" class="tools_btn">
            <span><b
                    style="background: url('/skins/Default/images/16/application.png') 50% 4px no-repeat;">显示</b></span>
        </a>
        <div class="tools_separator"></div>
        ...
        <div class="tools_separator"></div>
        <a title="" onclick="fun_ForceDelete();" class="tools_btn">
            <span><b
                    style="background: url('/skins/Default/images/16/application_delete.png') 50% 4px no-repeat;">强制删除</b></span>
        </a>
...
<script type="text/javascript">
    $(function () {
        $('#tt').datagrid({
            ...
            nowrap: false,
            striped: true,
            url: '/GLPT/DM/DevelopingPartyFindPage.do',
            singleSelect: true,
            remoteSort: false,
            rownumbers: true,
            idField: 'Id',
    ...
    var fun_del = function () {
        ...
        if (row) {
            delConfig("/GLPT/DM/DevelopingPartyThoroughDelOK.do", "Id=" + row.Id);
        } else {
            showFaceMsg("请选择要操作的记录");
        }
    }

    var fun_ForceDelete = function () {
        var row = $('#tt').datagrid('getSelected');
        if (row) {
            delConfig("/GLPT/DM/DevelopingPartyForceThoroughDelOK.do", "Id=" + row.Id);
        } else {
            showFaceMsg("请选择要操作的记录");
        }
    }
